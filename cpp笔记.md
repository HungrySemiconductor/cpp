...
- 结构体的**指针存储数值的起始地址**，使用**索引访问数值内容**

  ```c++
  // "mat.pdata 存储着矩阵数值的起始地址"
  // "通过 mat.pdata[index] 来访问具体位置的数值"
  
  Struct Matrix{
  	int rows;
  	int cols;
  	float *pData;	//矩阵中元素数值存储的地址
  };
  ...
  mat.rows = 2;
  mat.cols = 2;
  mat.pdata = new float[4];  // 分配4个float的空间
  
  // 现在内存情况：
  // pdata → [地址A] → 第一个float的位置
  //         [地址A+4] → 第二个float的位置  
  //         [地址A+8] → 第三个float的位置
  //         [地址A+12] → 第四个float的位置
  
  // 设置数据
  mat.pdata[0] = 1.0f;  // 第一个元素
  mat.pdata[1] = 2.0f;  // 第二个元素  
  mat.pdata[2] = 3.0f;  // 第三个元素
  mat.pdata[3] = 4.0f;  // 第四个元素
  
  // 实际发生了什么？
  // mat.pdata[0] 等价于 *(mat.pdata + 0)
  // mat.pdata[1] 等价于 *(mat.pdata + 1)
  // mat.pdata[2] 等价于 *(mat.pdata + 2)
  ```
...
- 三种调用方式
  ...
  - 先声明，后调用（声明的函数顺序不用管）
  - 直接调用（被调用的函数必须在当前函数的前面）
  - 函数声明全部放在头文件里

- 模块化编程

  ![image-20251012103416301](D:\GitHub\cpp\images\image-20251012103416301.png)
...
- 传参的两种方式
  ...
  - **pass by reference**（引用传递）
    ...
    >
    > **但要注意避免直接修改原变量，可以使用const来限制参数从而避免修改**

## 6.2 函数的返回

**进入函数先做输入输出值的检查（限定），避免函数执行时发生错误**

- 无具体返回值：void函数
- 有具体返回值（**值传递**）：返回基本数据类型

## 6.3 内联函数（Inline functions）

- Inline：编译器根据情况选择是是否将带有Inline标识的函数作为内联函数（即为提高效率，而将函数内容直接嵌入，而非调用）
- 宏：编译之前预处理（编译之前做文本替换），同样也可以减少编译的代价
